    ...
    <Card>
      ...
      <CardContent className="space-y-2">
        ...
        <div className="flex items-center gap-2">
          <Button size="sm" className="h-7 text-xs" onClick={onReload} disabled={loading}>
            {loading ? 'Fetching…' : 'Reload tags'}
          </Button>
        </div>
        <p className="text-[10px] text-muted-foreground leading-tight">
          <code>key: otag:query</code> per line. Fetched live from Scryfall and cached for the
          session, so the vocabulary is a knob rather than a redeploy.
        </p>
