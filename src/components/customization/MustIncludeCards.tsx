...
export function MustIncludeCards() {
  ...
  }, [query, colorIdentity, mustIncludeCards, bannedCards]);
  ...
  const handleImportList = () => {
    ...
    for (const line of lines) {
      ...
      if (cardName && !mustIncludeCards.includes(cardName) && !bannedCards.includes(cardName) && !newCards.includes(cardName)) {
    ...
    if (newCards.length > 0) {
      updateCustomization({
        mustIncludeCards: [...mustIncludeCards, ...newCards],
      });
    }
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Must Include Cards List */}
      {mustIncludeCards.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {mustIncludeCards.map((cardName) => (
            <span
              key={cardName}
              className="inline-flex items-center gap-1 px-2 py-1 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-xs rounded-md border border-emerald-500/20"
            >
      ...
      {mustIncludeCards.length === 0 && !showImport && (
        <p className="text-xs text-muted-foreground">
